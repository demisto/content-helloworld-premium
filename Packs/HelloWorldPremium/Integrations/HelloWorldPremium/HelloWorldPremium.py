"""HelloWorldPremium Integration for Cortex XSOAR (aka Demisto)
...
The HelloWorldPremium API is a simple API that shows a realistic use case for an XSOAR
...
This function is used in conjunction of the HelloWorldPremium Scan playbook that uses
...
and the prefix (i.e. ``HelloWorldPremium.Alert``) is passed via the ``outputs_prefix``
argument.
...
the list of entities (i.e. in HelloWorldPremium you want to update the status of an
existing ``HelloWorldPremium.Alert`` in the context when you retrieve it, rather than
...
class Client(BaseClient):
    ...
    For this HelloWorldPremium implementation, no special attributes defined
    """
    ...
    def search_alerts(self, alert_status: Optional[str], severity: Optional[str],
        ...
        """Searches for HelloWorldPremium alerts using the '/get_alerts' API endpoint
        ...
        :return: list containing the found HelloWorldPremium alerts as dicts
    ...
    def get_alert(self, alert_id: str) -> Dict[str, Any]:
        """Gets a specific HelloWorldPremium alert by id
    ...
    def update_alert_status(self, alert_id: str, alert_status: str) -> Dict[str, Any]:
        """Changes the status of a specific HelloWorldPremium alert
    ...
    def scan_start(self, hostname: str) -> Dict[str, Any]:
        """Starts a HelloWorldPremium scan on a specific hostname
    ...
    def scan_status(self, scan_id: str) -> Dict[str, Any]:
        """Gets the status of a HelloWorldPremium scan
    ...
    def scan_results(self, scan_id: str) -> Dict[str, Any]:
        """Gets the results of a HelloWorldPremium scan
...
def convert_to_demisto_severity(severity: str) -> int:
    """Maps HelloWorldPremium severity to Cortex XSOAR severity

    Converts the HelloWorldPremium alert severity level ('Low', 'Medium',
    'High', 'Critical') to Cortex XSOAR incident severity (1 to 4)
    for mapping.

    :type severity: ``str``
    :param severity: severity as returned from the HelloWorldPremium API (str)
...
def test_module(client: Client, first_fetch_time: int) -> str:
    ...
    :type client: ``Client``
    :param Client: HelloWorldPremium client to use
...
def say_hello_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    # INTEGRATION DEVELOPER TIP
    # In this case 'name' is an argument set in the HelloWorldPremium.yml file as mandatory,
...
def fetch_incidents(client: Client, max_results: int, last_run: Dict[str, int],
    ...
    :type client: ``Client``
    :param Client: Hello World Premium client to use
    ...
    for alert in alerts:
        ...
        # convert it from the Hello World Premium API response
        ...
        # "type") or custom fields (like "helloworldpremiumid") directly here in the
        ...
        incident = {
            ...
            # 'type': 'Hello World Premium Alert',  # Map to a specific XSOAR incident Type
            'severity': convert_to_demisto_severity(alert.get('severity', 'Low')),
            # 'CustomFields': {  # Map specific XSOAR Custom Fields
            #     'helloworldpremiumid': alert.get('alert_id'),
            #     'helloworldpremiumstatus': alert.get('alert_status'),
            #     'helloworldpremiumtype': alert.get('alert_type')
            # }
        }
...
def ip_reputation_command(client: Client, args: Dict[str, Any], default_threshold: int) -> List[CommandResults]:
    ...
    for ip in ips:
        ...
        # HelloWorldPremium score to XSOAR reputation mapping
        ...
        # parts: the vendor-specific context (HelloWorldPremium), the standard-context
        # (IP) and the DBotScore.
        # More information:
        # https://xsoar.pan.dev/docs/integrations/context-and-outputs
        # https://xsoar.pan.dev/docs/integrations/context-standards
        # https://xsoar.pan.dev/docs/integrations/dbot
        # Also check the HelloWorldPremium Design Document
...
def search_alerts_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    """helloworldpremium-search-alerts command: Search alerts in HelloWorldPremium
...
def get_alert_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    """helloworldpremium-get-alert command: Returns a HelloWorldPremium alert
    ...
    readable_output = tableToMarkdown(f'HelloWorldPremium Alert {alert_id}', alert)
...
def update_alert_status_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    """helloworldpremium-update-alert-status command: Changes the status of an alert

    Changes the status of a HelloWorldPremium alert and returns the updated alert info
...
def scan_start_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    """helloworldpremium-start-scan command: Starts a HelloWorldPremium scan
...
def scan_status_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    """helloworldpremium-scan-status command: Returns status for HelloWorldPremium scans
...
def scan_results_command(client: Client, args: Dict[str, Any]) -> Union[Dict[str, Any], CommandResults, List[CommandResults]]:
    """helloworldpremium-scan-results command: Returns results for a HelloWorldPremium scan
...
def main() -> None:
    ...
    try:
        ...
        elif demisto.command() == 'helloworldpremium-say-hello':
            return_results(say_hello_command(client, demisto.args()))

        elif demisto.command() == 'helloworldpremium-search-alerts':
            return_results(search_alerts_command(client, demisto.args()))

        elif demisto.command() == 'helloworldpremium-get-alert':
            return_results(get_alert_command(client, demisto.args()))

        elif demisto.command() == 'helloworldpremium-update-alert-status':
            return_results(update_alert_status_command(client, demisto.args()))

        elif demisto.command() == 'helloworldpremium-scan-start':
            return_results(scan_start_command(client, demisto.args()))

        elif demisto.command() == 'helloworldpremium-scan-status':
            return_results(scan_status_command(client, demisto.args()))

        elif demisto.command() == 'helloworldpremium-scan-results':
