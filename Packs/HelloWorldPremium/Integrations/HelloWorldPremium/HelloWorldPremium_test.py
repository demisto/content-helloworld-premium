"""HelloWorldPremium Integration for Cortex XSOAR - Unit Tests file

This file contains the Unit Tests for the HelloWorldPremium Integration based
...
demisto-sdk lint -i Packs/HelloWorldPremium/Integrations/HelloWorldPremium
...
before each test to simulate the API calls to the HelloWorldPremium API. This way we
...
def test_say_hello():
    """Tests helloworldpremium-say-hello command function.
    ...
    any external API.
    """
    from HelloWorldPremium import Client, say_hello_command
...
def test_start_scan(requests_mock):
    """Tests helloworldpremium-scan-start command function.
    ...
    """
    from HelloWorldPremium import Client, scan_start_command
    ...
    assert response.outputs_prefix == 'HelloWorldPremium.Scan'
...
def test_status_scan(requests_mock):
    """Tests helloworldpremium-scan-status command function.
    ...
    """
    from HelloWorldPremium import Client, scan_status_command
...
def test_scan_results(mocker, requests_mock):
    """Tests helloworldpremium-scan-results command function.

    Configures requests_mock instance to generate the appropriate
    get_scan_results API response, loaded from a local JSON file. Checks
    the output of the command function with the expected output.
    """
    from HelloWorldPremium import Client, scan_results_command
    ...
    assert response[0].outputs_prefix == 'HelloWorldPremium.Scan'
    assert response[0].outputs_key_field == 'scan_id'

    # This command also returns Common.CVE data
    assert len(response) > 0
    assert isinstance(response[1].indicator, Common.CVE)


def test_search_alerts(requests_mock):
    """Tests helloworldpremium-search-alerts command function.

    Configures requests_mock instance to generate the appropriate
    get_alerts API response, loaded from a local JSON file. Checks
    the output of the command function with the expected output.
    """
    from HelloWorldPremium import Client, search_alerts_command
    ...
    assert response.outputs_prefix == 'HelloWorldPremium.Alert'
    assert response.outputs_key_field == 'alert_id'
    assert response.outputs == mock_response['alerts']


def test_get_alert(requests_mock):
    """Tests helloworldpremium-get-alert command function.

    Configures requests_mock instance to generate the appropriate
    get_alerts API response, loaded from a local JSON file. Checks
    the output of the command function with the expected output.
    """
    from HelloWorldPremium import Client, get_alert_command
...
def test_update_alert_status(requests_mock):
    """Tests helloworldpremium-update-alert-status command function.

    Configures requests_mock instance to generate the appropriate
    get_alerts API response, loaded from a local JSON file. Checks
    the output of the command function with the expected output.
    """
    from HelloWorldPremium import Client, update_alert_status_command
...
def test_ip(requests_mock):
    ...
    the output of the command function with the expected output.
    """
    from HelloWorldPremium import Client, ip_reputation_command
    from CommonServerPython import Common
    ...
    assert response[0].outputs == mock_response
    assert response[0].outputs_prefix == 'HelloWorldPremium.IP'
...
def test_domain(requests_mock):
    ...
    the output of the command function with the expected output.
    """
    from HelloWorldPremium import Client, domain_reputation_command
    from CommonServerPython import Common
    ...
    assert response[0].outputs == mock_response
    assert response[0].outputs_prefix == 'HelloWorldPremium.Domain'
...
def test_fetch_incidents(requests_mock):
    ...
    the output of the command function with the expected output.
    """
    from HelloWorldPremium import Client, fetch_incidents
    ...
    assert new_incidents == [
        {
            'name': 'Hello World Premium Alert 100',
        ...
        {
            'name': 'Hello World Premium Alert 200',
            'occurred': '2020-02-17T23:34:23.000Z',
